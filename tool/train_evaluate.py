...
class Trainer:
    ...
    def fit(self, filename, is_chirps=False):
        ...
        for epoch in range(1,self.epochs+1):
            ...
            evaluator = Evaluator(self.model, self.loss_fn, self.optimizer, self.val_loader, None, None, self.device, self.util)
            val_loss,_ = evaluator.eval(is_test=False, is_chirps=is_chirps)
            if (self.verbose):
                print(f'Epoch: {epoch}/{self.epochs} - loss: {train_loss:.6f} - val_loss: {val_loss:.6f}')
...
class Evaluator:
    ...
    def __init__(self, model, loss_fn, optimizer, data_loader, train_min, train_max, device, util=None, step=0):
        self.model = model
        self.loss_fn = loss_fn
        self.optimizer = optimizer
    ...
    def eval(self, is_test=True, is_chirps=False):
        ...
        with torch.no_grad(): 
            for batch_i, (inputs, target) in enumerate(self.data_loader):
                ...
                # denormalize if it is test dataset
                if is_test:
                    output = ( output * (self.train_max - self.train_min) ) + self.train_min
                    target = ( target * (self.train_max - self.train_min) ) + self.train_min
    ...
    def load_checkpoint(self, filename, dataset_type=None, model=None):
        ...
        if os.path.isfile(filename):
            ...
            print(f'=> Loaded checkpoint {name} (best epoch: {epoch}, validation mse: {loss:.4f})')
        else:
